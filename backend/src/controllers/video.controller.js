...
const publishAVideo = asyncHandler(async (req, res) => {
   ...
   const {ThumbnailFilePath} = req.files?.thumbnail?.[1]?.path
  ...
  const videoUrl = await uploadeOnCloudinary(VideoFilePath)
  const thumbnailUrl = await uploadeOnCloudinary(ThumbnailFilePath)
